...
function encodedFile(sourceFile){
  ...
  try {
    ...
    console.log(`Source file encoded to Hex. New file created:  ${encodedHexFile}`)
    fs.writeFileSync(encodedBase64File, encodedBase64)
    console.log(`Source file encoded to Base64. New file created:  ${encodedBase64File}`)
  } catch (err) {
    console.error(err)
  }
} 
...
function encryptFile(encodedHexFile, encodedBase64File){
  ...
  try {
    ...
    console.log(`Hex file encrypted. New file created:  ${encryptedBase64File}`)
  } catch (err) {
    console.error(err)
  }
...
function decryptFile(encryptedHexFile){
    ...
    try {
      ...
      console.log(`Encrypted Hex file decrypted and decoded - saved to a new file: ${decodedHexFile}`)
    } catch (err) {
      console.error(err)
    }
}

// encode, encrypt and decrypt tests: see https://github.com/klarema/sprint_1/blob/main/README.md
